...
main(int argc, char **argv)
{
  ...
  int a =0;
  if (!ParseArguments(argc, argv, &input_scene_name, &output_name, a))
    return -1;
  ...
  // cout<< typeid(scene).name();
  SurfaceList::Ptr s = dynamic_pointer_cast<SurfaceList>(scene);
  const vector<Surface::Ptr> &surfaces = s->GetSurfaces();

  BVHNode::Ptr bvh_tree = BVHNode::BuildBVH(surfaces);
  RayTracer rt;

...
public:
  ...
  //! \brief Set num samples per pixel for use in antialiasing calculations
  //! \param[in] spp Number of samples per pixel (bounce count)
  inline void SetNumSamplesPerPixel(uint spp) {spp_ = spp;}
...
protected:
  ...
  uint spp_{1}; //!< num samples per pixel

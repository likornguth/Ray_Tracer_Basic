...
ImageTexture::Value(const Vec2r &uv, const Vec3r &/*position*/)
{
  ...
  // auto s = static_cast<float>(x-xi);
  // auto t = static_cast<float>(y-yi);
  // auto a = image_.at<cv::Vec3f>(yi,xi);
  // auto b = image_.at<cv::Vec3f>(yi,xi+1);
  // auto c = image_.at<cv::Vec3f>(yi+1, xi);
  // auto d = image_.at<cv::Vec3f>(yi+1, xi+1);
  // auto color = (1-s)*(1-t)*a + s*(1-t)*b + (1-s)*t*c + s*t*d;

  Vec3r value{color[2]/255.0, color[1]/255.0, color[0]/255.0};
  // cout << color;
  
  return (value.cwiseProduct(gain_) + bias_);
